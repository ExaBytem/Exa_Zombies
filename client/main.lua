...
RegisterCommand("mutezombies", function(source, args, rawCommand)
	if muted ~= true then
		muted = true
		ESX.ShowNotification(_U('mute'))
		Citizen.Wait(10000)
	else
		muted = elsi
		ESX.ShowNotification(_U('unmute'))
		Citizen.Wait(10000)
		end
 end)
...
RegisterNetEvent("Exa_Zombiesplayerupdate")
AddEventHandler("Exa_Zombiesplayerupdate", function(mPlayers)
	players = mPlayers
end)
...
TriggerServerEvent("Exa_Zombiesnewplayer", PlayerId())
...
AddEventHandler("ZombieSync", function()
	...
	while true do
		...
		if #entitys < Config.SpawnZombie then
			...
			zsound = sounds[math.random(1, #sounds)]
	...
	while true do
		...
		for i, entity in pairs(entitys) do
			for j, player in pairs(players) do
				...
				if distance <= 25.0 and distance > 10 then
					...
					local hel = GetEntityHealth(entity)
				elseif distance <= 10.0 and muted ~= true then
					xsound = SedaHa[math.random(1, #SedaHa)]
					TriggerServerEvent('Exa_Zombies:Sound:PlayWithinDistance', 0.5, xsound, 1.0)
					Citizen.Wait(3000)
				end
			end
		end
				...
				if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 0.6)then
					if IsPedRagdoll(entity, 1) ~= 1 then
						if not IsPedGettingUp(entity) then
							PlayAmbientSpeech1 (entity, sounds, "SPEECH_PARAMS_FORCE", 1)
					...
					if GetPedSourceOfDeath(entity) == PlayerPedId() then
						...
						if not IsPedInAnyVehicle(PlayerPedId(), false) then
							if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 1.5) then
								ESX.Game.Utils.DrawText3D({x = pedX, y = pedY, z = pedZ + 0.2}, '~b~PRESS ~r~[E]~b~ TO SEARCH', 0.8, 4)
								if IsControlJustReleased(1, 51) then
									if DoesEntityExist(GetPlayerPed(-1)) then
										...
										elseif randomChance >= Config.ProbabilityWeaponLootObject and randomChance < Config.ProbabilityItemLootObject then
											TriggerServerEvent('Exa_Zombiesitemloot', randomItem)
										elseif randomChance >= Config.ProbabilityItemLootObject and randomChance < Config.ProbabilityMoneyLootObject then
											TriggerServerEvent('Exa_Zombiesmoneyloot')
										elseif randomChance >= Config.ProbabilityMoneyLootObject and randomChance < 100 then
											ESX.ShowNotification(_U('noting_found'))
										end
...
RegisterNetEvent('Exa_Zombiesclear')
AddEventHandler('Exa_Zombiesclear', function()
	for i, entity in pairs(entitys) do
		local model = GetEntityModel(entity)
		SetEntityAsNoLongerNeeded(entity)
		SetModelAsNoLongerNeeded(model)
...
RegisterNetEvent('Exa_Zombies:Sound:PlayOnOne')
AddEventHandler('Exa_Zombies:Sound:PlayOnOne', function(soundFile, soundVolume)
    SendNUIMessage({
...
RegisterNetEvent('Exa_Zombies:Sound:PlayOnAll')
AddEventHandler('Exa_Zombies:Sound:PlayOnAll', function(soundFile, soundVolume)
    SendNUIMessage({
        transactionType     = 'playSound',
        transactionFile     = soundFile,
        transactionVolume   = soundVolume
...
RegisterNetEvent('Exa_Zombies:Sound:PlayWithinDistance')
AddEventHandler('Exa_Zombies:Sound:PlayWithinDistance', function(playerNetId, maxDistance, soundFile, soundVolume)
